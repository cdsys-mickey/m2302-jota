...
const RecvAccCustomerPicker = (props) => {
	const {
		label = "零售客戶",
		forId,
		sess,
	...
	const querystring = useMemo(() => {
		return queryString.stringify({
			tp: 10000,
			ym: session?.CurAccYM,
			sess: session?.CurStage
		});
	}, [session?.CurAccYM, session?.CurStage]);
	...
	const url = useMemo(() => {
		return "v1/sales/recv-account/customers"
	}, [])
...
RecvAccCustomerPicker.displayName = "RecvAccCustomerPicker";
RecvAccCustomerPicker.propTypes = {
...
export default RecvAccCustomerPicker;