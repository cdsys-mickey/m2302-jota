...
export const useP21 = ({ token }) => {
	...
	const loadItem = useCallback(
		async ({ id, refresh }) => {
			...
			try {
				...
				if (status.success) {
					const data = P21.transformForReading(payload.data[0]);
	...
	const onEditorSubmit = useCallback(
		async (data) => {
			console.log(`P21.onEditorSubmit()`, data);

			const processed = P21.transformForEditorSubmit(data);
	...
	const onSearchSubmit = useCallback(
		(data) => {
			...
			// const q = data?.q;
			listLoader.loadList({
				params: P21.transformAsQueryParams(data),
				// reset: true,
			});
		},
