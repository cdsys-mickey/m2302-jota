...
import { CaptchaWrapper } from "./local-captcha-styles";
...
function genetateNumericCaptcha(max) {
	let text = "";
	let i;
	for (i = 0; i < max; i += 1) {
...
class LocalCaptcha extends Component {
	state = {
		solution: this.props.numbersOnly
			? genetateCaptcha(this.props.length)
			: genetateNumericCaptcha(this.props.length),
		input: "",
	};
	...
	drawCaptcha = () => {
		...
		ctx.lineTo(
			getRandomInt(width / 2, width / 2 + 10),
			getRandomInt(5, 20)
		);
		ctx.stroke();
		ctx.closePath();
		...
		this.setState(
			{
				...
				solution: genetateNumericCaptcha(this.props.length),
				input: "",
			},
			() => this.drawCaptcha({ strokeStyle: this.props.strokeStyle })
		);
		onRefresh();
	};
...
LocalCaptcha.defaultProps = {
	placeholder: "Insert captcha",
	length: 6,
	// strokeStyle: "purple",
};

LocalCaptcha.propTypes = {
