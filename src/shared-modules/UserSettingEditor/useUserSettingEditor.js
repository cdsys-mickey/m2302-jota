...
export default function useUserSettingEditor({ id, moduleId, scope = Settings.SCOPES.USER, defaultValues = {} }) {
	...
	const loadItem = useCallback(
		async ({ refresh }) => {
			...
			try {
				...
				if (status.success) {
					const data = UserSettingEditors.transformForReading(payload);
				...
				const defaultData = UserSettingEditors.transformForReading(defaultValues);
				switch (err?.status) {
					case 404:
	...
	const onSubmit = useCallback(async (payload) => {
		...
		const data = UserSettingEditors.transformForGridSubmit(grid.gridData);
