...
export const useD07 = () => {
	...
	const loadItem = useCallback(
		async ({ id, refresh = false }) => {
			try {
				...
				if (status.success) {
					const data = D07.transformForReading(payload.data[0]);
	...
	const confirmDelete = useCallback(() => {
		dialogs.confirm({
			message: `確認要删除試算單「${itemData?.CalID}」?`,
			onConfirm: async () => {
				try {
					...
					if (status.success) {
						// 關閉對話框
						crud.cancelAction();
						toastEx.success(`成功删除試算單 ${itemData?.CalID}`);
						listLoader.loadList({ refresh: true });
					} else {
						throw error || `發生未預期例外`;
					}
	...
	const onEditorSubmit = useCallback(
		(data) => {
			...
			const collected = D07.transformForSubmitting(data, grid.gridData);
	...
	const reportUrl = useMemo(() => {
		return `${config.REPORT_URL}/WebD07Rep.aspx`;
	}, [config.REPORT_URL]);
	...
	const onPrintSubmit = useCallback(
		(payload) => {
			...
			const data = {
				...
				JobName: "D07",
				IDs: crud.itemData?.CalID,
			};
			// console.log("jsonData", data);
			// postToBlank(
			// 	`${config.REPORT_URL}/WebD07Rep.aspx?LogKey=${operator?.LogKey
			// 	}`,
			// 	{
